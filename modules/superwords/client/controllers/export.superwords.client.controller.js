...
// Export Superword controller
angular.module('superwords').controller('ExportSuperwordsController', ['$scope', '$stateParams', '$location', 'Authentication', 'Superwords', 'SuperwordGroups', '$http', '$q', '$window', 'Download',
  function ($scope, $stateParams, $location, Authentication, Superwords, SuperwordGroups, $http, $q, $window, Download) {
    $scope.authentication = Authentication;

    // init
    var templatePath = "modules/superwords/client/template";
    ...
    // init superword group grid
    $scope.superwordGroups = SuperwordGroups.query();
    $scope.superwordGroupGridOptions = {
      // Sort
      enableSorting: false,
      // Select
    ...
    $scope.superwordGroupGridOptions.onRegisterApi = function (gridApi) {
      $scope.superwordGroupGridApi = gridApi;
    };
    ...
    $scope.exportPackage = function(resourceDirectory,template) {
      ...
      // get superwords of group
      var getSuperwordsWithGroup = function (group) {
        var groupId = group._id;

        var deferred = $q.defer();

        // get superwords data
        Superwords.getFromGroup({
          superwordGroupId: groupId
        }, function (superwords)
        {
          group.superwords=superwords;
      ...
      // get superwords of each group
      var rows = $scope.superwordGroupGridApi.grid.rows;
      var promises = [];
      for (var i = 0; i < rows.length; i++) {
        var row = rows[i];
        var group = row.entity;
        var promise = getSuperwordsWithGroup(group);
        promises.push(promise);
      }
